...
public final class Checker implements Visitor {
   ...
   private TryCommand currentTry = null;
  // Commands
  ...
  public Object visitForCommand(ForCommand ast, Object o) {
    // Aquí más adelante generamos código TAM
    return null;
  }
  ...
  if (currentTry != null && currentTry.type != null) {
    if (!exprType.equals(currentTry.type)) {
      reporter.reportError("Tipo de la expresión en 'throw' no coincide con el tipo del 'catch'", "", ast.position);
    }
  } else {
    reporter.reportError("'throw' debe estar dentro de un bloque 'try'", "", ast.position);
  }
  ...
  // offset due to tdexed ishese indexing operations at run-time.
