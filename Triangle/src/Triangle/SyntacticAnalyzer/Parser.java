...
public class Parser {
  ...
  Command parseSingleCommand() throws SyntaxError {
    ...
    switch (currentToken.kind) {
      ...
      case Token.FOR: {
        acceptIt(); // acepta 'for'
        Vname iAST = parseVname(); // la variable de control del for
        accept(Token.BECOMES); // acepta :=
        ...
        accept(Token.DO); // 'do'
        Command cAST = parseSingleCommand(); // cuerpo del for
  ...
  Declaration parseDeclaration() throws SyntaxError {
    ...
    // Corrección importante: si viene un 'in' después de la primera declaración, no
    // seguir
    while (currentToken.kind == Token.SEMICOLON) {
      acceptIt();
      if (currentToken.kind == Token.IN) // <<--- Esto se agregó para permitir LET correctamente
        break;
